...
gesture_map = {
    ...
    "comfort": "animations/Stand/Gestures/CalmDown_1",
    "think": "animations/Stand/Gestures/Thinking_1",
    "nod": "animations/Stand/Gestures/Yes_1",
    "shrug": "animations/Stand/Gestures/Shrug_1",
    "none": None
}
...
class GPTDemo(SICApplication):
    ...
    def setup(self):
        ...
        self.conf = GPTConf(
            ...
            system_message="""
                You are Dr. Teddy, a funny but serious robot therapist.
                Always respond with valid JSON like this:

                {"text": "your answer", "gesture": "think"}

                you have gesture choice of: wave, laugh, comfort, think, nod, shrug, none
    ...
    def say_with_gesture(self, reply_data):
        ...
        self.nao.motion.request(NaoqiAnimationRequest(gesture_map[reply_data["gesture"]]), block=False)
        self.nao.tts.request(NaoqiTextToSpeechRequest(reply_data["text"]))
    ...
    def run(self):
        ...
        try:
            self.wakeup()
            self.logger.info("Started up successfully")
            # self.say_animated()
            sleep(1)
            ...
            while not self.shutdown_event.is_set() and i < self.NUM_TURNS:
                ...
                reply = self.gpt.request(GPTRequest(input=user_input, context_messages=self.context, system_message=self.conf.system_message))
                print("Reply: {response}".format(response=reply.response))
                reply_data = json.loads(reply.response)

                sleep(1)
                self.say_with_gesture(reply_data)
